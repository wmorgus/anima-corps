...
**Push-delivered telos fails (a).** §6a.10 leaves pull-vs-push open for context-in-general. Telos delivery is not context-in-general. Chain: (a) = §3.3 at the telos layer; §3.3 requires the validator external to the validated; §6a.10's own note says push collapses externality, pull preserves it; therefore push-delivered telos collapses the end into the executor and fails (a). (b) does not rescue it — (b) is silent on retrieval primitive, push and pull both satisfy readability if the delivered telos is structured. But (a) and (b) are not symmetric: (b)'s silence on primitive permits push; (a)'s externality requirement forbids it. The permissive requirement does not override the forbidding one. So even where §6a.10 leaves push open, MVT closes it at the telos layer: telos must be pulled, or held in a place the executor reads but does not author. Push at the telos layer is self-attestation at the goal level by another name.
